...
public sealed class ProposalTests
{
    ...
    {
        var p=Proposal.Create("دفع 250 جنيه",ProposalOperationType.PersonalExpense,Guid.NewGuid(),null,25000,"EGP",DateTimeOffset.UtcNow.AddMinutes(1),null,null,null,DateTimeOffset.UtcNow.AddHours(1));
        p.Status.Should().Be(ProposalStatus.ReadyForConfirmation);
    }

    [Fact]
    public void Missing_fields_keep_proposal_in_draft()
    {
